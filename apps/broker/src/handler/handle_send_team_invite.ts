...
import { SendTeamInviteEvent } from "../event/send_team_invite"
import { dlq_topic } from "../lib/topic"
import { deliver_email } from "../consumer/deliver_email"

export async function handle_send_team_invite(
  ...
  const validation = v.safeParse(SendTeamInviteEvent, raw)
  if (!validation.success) {
    logger.error("malformed send_team_invite event", {
