...
const EXPECTED_HEADER = "name\tadmin_level\tlabel\tbadge\tgeometry_hex"
...
function get_zone_files(): { country: string; path: string }[] {
  ...
  return readdirSync(ZONES_DIR, { withFileTypes: true })
    ...
    .filter(({ country, path }) => {
      if (!existsSync(path)) {
        console.log(`skipping ${country} — data.tsv not found (run just zone ${country} first)`)
        return false
      }
      const content = readFileSync(path, "utf-8")
      const header = content.split("\n")[0].trim()
      if (header !== EXPECTED_HEADER) {
        console.log(`skipping ${country} — data.tsv has unexpected header (regenerate with just zone ${country})`)
        return false
      }
      return true
    })
...
export async function seed(db: Kysely<DB>): Promise<void> {
  ...
  if (zone_files.length === 0) {
    console.log("skipping zones seed — no valid data in db/zones/")
    return
  }
  ...
  for (const { country, path } of zone_files) {
    ...
    for (const [name, admin_level, label, badge, geometry_hex] of rows) {
      await sql`
