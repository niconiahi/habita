# Surety Warranty Flow

## Scenario

A property manager lists a rental property and creates a contract backed by a **surety warranty** (garantía de caución). In this model, an insurance company issues a policy that guarantees the tenant's obligations — if the tenant defaults, the insurer covers the landlord. This is a common arrangement in Argentine rental markets as an alternative to property-based guarantees.

The manager creates the property, assigns a landlord, then fills the contract with all required sections. For the warranty, they select "surety" and provide the guarantor's personal details along with insurance company information (company name, policy number, company email). After generating the contract PDF, the property is published with visiting slots. A candidate books a slot and is then assigned as tenant by the manager.
...
1. **Creates a property** — Manager fills location, type (DEPARTMENT), unit "2B", destiny, submits → redirects to `/admin/properties/:id/edit`
2. **Assigns landlord** — DB operation: fetches landlord user ID, calls `assign_property_access` with `ACCESS_TYPE.LANDLORD`
3. **Creates and fills contract** — Manager navigates to `/admin/properties/:id/contracts/new`, sets price $120,000, fills all contract sections, selects "surety" warranty, fills guarantor details (María García), insurance company details (Seguros Test S.A., policy POL-2024-001234), generates PDF
4. **Publishes property** — Manager goes to `/admin/properties`, clicks publish for this property
5. **Creates visiting slots** — Manager navigates to `/admin/properties/:id/calendar`, creates slot 2 days from now (14:00–15:00)
