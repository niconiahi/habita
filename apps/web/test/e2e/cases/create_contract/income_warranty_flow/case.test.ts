...
import { TEST_LANDLORD } from "$test/helpers/auth"
import {
  fill_contract_sections,
  fill_income_warranty,
  fill_initial_price,
  generate_and_verify_pdf,
} from "$test/helpers/contract-form"
import {
...
test.describe.serial("Full Flow - INCOME Warranty", () => {
  ...
  test("3. Creates contract", async ({ page }) => {
    await page.goto(
  ...
  test("4. Fills contract sections", async ({ page }) => {
    await page.goto(
  ...
  test("5. Fills INCOME warranty", async ({ page }) => {
    await page.goto(
      `/admin/properties/${property_id}/contracts/${contract_id}/edit/sections`,
    )
  ...
  test("6. Fills initial price and generates PDF", async ({
    page,
  }) => {
    await page.goto(
