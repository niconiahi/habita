...
import { PROPERTY_DESTINY } from "$lib/property_destiny"
...
export async function create_test_contract_state(
  user_id: number,
): Promise<{ property_id: number; contract_id: number }> {
  const location_id = await seeder.create_location({
    address: "Test 123, CABA",
    latitude: -34.6037,
    longitude: -58.3816,
    road: "Test",
    house_number: 123,
