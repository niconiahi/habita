<script lang="ts">
  ...
  async function handle_input(event: Event) {
    ...
    const url = new URL("/nominatim/search", window.location.origin);
    url.searchParams.set("q", value);
    url.searchParams.set("addressdetails", String(1));

    const response = await fetch(url).catch(() => {
      throw new Error("unknown error while searching for an address");
    });

    if (!response.ok) {
      throw new Error("there was an error while searching for an address");
    }
...
<div>
  ...
  {/if}
  <input type="hidden" name="location" value={JSON.stringify(selected)} />
  {#if selected}
    <a
