<script lang="ts">
  ...
  interface Props {
    rooms: Room[];
    is_readonly?: boolean;
    on_positions_change?: (positions: Map<number, Position>) => void;
  }

  let { rooms, is_readonly = false, on_positions_change }: Props = $props();

  let canvas_el: HTMLCanvasElement | undefined = $state();
  let room_positions = $state<Map<number, Position>>(new Map());
  ...
  function get_snapped_position(
    ...
    for (const other of all_rooms) {
      ...
      if (Math.abs(room_rect.right - other_rect.left) < snap_threshold) {
        snapped_x = other_rect.left - dragging_room.w;
      } else if (Math.abs(room_rect.left - other_rect.right) < snap_threshold) {
        snapped_x = other_rect.right;
      }
      if (Math.abs(room_rect.bottom - other_rect.top) < snap_threshold) {
        snapped_y = other_rect.top - dragging_room.h;
      } else if (Math.abs(room_rect.top - other_rect.bottom) < snap_threshold) {
        snapped_y = other_rect.bottom;
      }
      if (Math.abs(room_rect.top - other_rect.top) < snap_threshold) {
        snapped_y = other_rect.top;
      } else if (
        Math.abs(room_rect.bottom - other_rect.bottom) < snap_threshold
      ) {
        snapped_y = other_rect.bottom - dragging_room.h;
      }
      if (Math.abs(room_rect.left - other_rect.left) < snap_threshold) {
        snapped_x = other_rect.left;
      } else if (
        Math.abs(room_rect.right - other_rect.right) < snap_threshold
      ) {
  ...
  function update_room_position(room_id: number, x: number, y: number): void {
  ...
  function handle_pointer_down(event: PointerEvent): void {
    ...
    const room = get_room_at_point(canvas_x, canvas_y, room_bounds, PADDING);
    if (room) {
  ...
  function handle_pointer_move(event: PointerEvent): void {
    ...
    if (!has_collision) {
      update_room_position(dragging_state.room_id, new_x, new_y);
    }
  }
  ...
  $effect(() => {
    ...
    const total_width = room_data.reduce((sum, room) => sum + room.width, 0);
    const max_length = Math.max(...room_data.map((r) => r.length), 1);
    const default_available_width = 600 - padding * 2;
    const default_available_height = 300 - padding * 2;
    const scale_x = default_available_width / (total_width || 1);
    ...
    const colors = ["#d0d0d0", "#a0c4ff", "#ffadad", "#c9f0c9", "#ffe4b3"];
    ctx.save();
    ...
    for (const room of room_data) {
      const w = room.width * scale;
      const h = room.length * scale;
      const position = room_positions.get(room.id) || { x: x_offset, y: 0 };
