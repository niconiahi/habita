...
function warn(message: string, attributes?: LogAttributes): void {
...
function info(message: string, attributes?: LogAttributes): void {
  const span = trace.getActiveSpan();
  const span_context = span?.spanContext();
  const _attributes: LogAttributes = {
    ...attributes
...
function error(message: string, attributes?: LogAttributes, error?: Error) {
  const span = trace.getActiveSpan();
  const span_context = span?.spanContext();
  const _attributes: LogAttributes = {
    ...attributes
