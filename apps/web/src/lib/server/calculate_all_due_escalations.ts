...
import type { FormulaParameter, Period } from "$lib/server/db/types";
...
async function fetch_rates() {
  return query_builder.selectFrom("rate").selectAll().execute();
}
export type Rate = NonNullable<Awaited<ReturnType<typeof fetch_rates>>[0]>;
...
function get_due_contracts(contracts: Contract[]) {
  ...
  return contracts.filter((contract) => {
    ...
    if (!latest_period) {
      throw new Error("at least one period should exist for the contract");
    }
    const end_date = v.parse(ForceDateSchema, latest_period.end_date);
...
export async function calculate_all_due_escalations() {
  ...
  for (const contract of due_contracts) {
    ...
    const period = make_period(contract, latest_period, next_price);
    next_periods.push(period);
    const formula_parameters = make_formula_parameters(current_rate, old_rate);
  ...
  await query_builder.transaction().execute(async (trx) => {
    ...
    for (let i = 0; i < inserted_periods.length; i++) {
      next_formula_parameters[i * 2].period_id = inserted_periods[i].id;
      next_formula_parameters[i * 2 + 1].period_id = inserted_periods[i].id;
    }
    await trx
...
function get_current_rate(contract: Contract, rates: Rate[]): Rate {
...
function get_next_price(
  ...
  const result =
    current_price *
    (Number.parseFloat(current_rate) / Number.parseFloat(old_rate));
...
function make_period(
  ...
  const prev_end_date = v.parse(ForceDateSchema, latest_period.end_date);
...
function parse_duration_to_months(duration: Duration): number {
...
function add_duration(date: Date, duration: Duration): Date {
  const months_match = duration.match(/P(\d+)M/);
  if (months_match) {
    return addMonths(date, Number.parseInt(months_match[1], 10));
  }
  const years_match = duration.match(/P(\d+)Y/);
  if (years_match) {
    return addMonths(date, Number.parseInt(years_match[1], 10) * 12);
  }
  ...
  if (weeks_match) {
    return addDays(date, Number.parseInt(weeks_match[1], 10) * 7);
  }
  throw new Error(`Unsupported duration format: ${duration}`);
}
