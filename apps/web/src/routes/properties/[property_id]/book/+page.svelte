...
<Content.Root>
  ...
  <Content.Section>
    ...
    <Formulary.Root method="POST" action={compose_action(ACTION.SET_DATE)}>
      <Formulary.Fields>
        <fieldset>
          <legend>Fechas disponibles</legend>
          <div class="flex flex-col gap-2">
            {#each data.dates as date_item}
              {@const date_string = get_date(date_item.date)}
              <label class="flex items-center gap-2">
                <input type="radio" name="date" value={date_string} />
        ...
        {#if form?.errors && "set_date" in form.errors && form.errors.set_date?.date}
          <span class="text-red-500">{form.errors.set_date.date}</span>
        {/if}
      </Formulary.Fields>
      <Formulary.Actions>
  ...
  {#if data.times.length > 0}
    <Content.Section>
      ...
      <Formulary.Root method="POST" action={compose_action(ACTION.UPDATE_SLOT)}>
        <Formulary.Fields>
          <fieldset>
            <legend class="sr-only">Seleccionar un horario</legend>
            <div class="flex flex-col gap-2">
              {#each data.times as time (time.id)}
                <label class="flex items-center gap-2">
                  <input type="radio" name="id" value={time.id} />
                  <time datetime={time.start_date.toISOString()}>
          ...
          {#if form?.errors && "update_slot" in form.errors && form.errors.update_slot?.id}
            <span class="text-red-500">{form.errors.update_slot.id}</span>
          {/if}
          <input type="hidden" value={data.user.id} name="visitant_id" />
        </Formulary.Fields>
        <Formulary.Actions>
          <Button type="submit">Reservar este horario</Button>
        </Formulary.Actions>
      </Formulary.Root>
    </Content.Section>
  {/if}
