...
import { escape_ics_text, format_ics_date } from "$lib/server/ics";
...
async function execute(form_data: FormData, span: Span) {
  ...
  if (!property) {
    ...
    throw new Response("property should exist", { status: 404 });
  }
  span.setAttribute("property.location", display_location(property.location));
