...
import { RateTypeSchema, get_rate_types } from "$lib/rate_type";
...
export const actions: Actions = {
  default: async ({ request, locals }) => {
    ...
    const month = v.parse(ForceNumberSchema, form_data.get("month"));
    const year = v.parse(ForceNumberSchema, form_data.get("year"));
    ...
    await query_builder
      ...
      .onConflict((oc) =>
        oc.constraint("rate_type_month_year_unique").doUpdateSet({
          value,
          updated_at: now
        })
      )
