...
export async function create_file(form_data: FormData) {
  const contract_id = v.parse(ForceNumberSchema, form_data.get("contract_id"));
  const file_type = v.parse(
    ContractFileTypeSchema,
    Number(form_data.get("file_type"))
  );
  const file_ = v.parse(v.instance(File), form_data.get("file"));
  await query_builder.transaction().execute(async (tx) => {
    const content = Buffer.from(await file_.arrayBuffer());
    const hash_buffer = await crypto.subtle.digest("SHA-256", content);
