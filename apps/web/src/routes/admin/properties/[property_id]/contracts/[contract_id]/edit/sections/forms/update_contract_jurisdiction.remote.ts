...
export const update_contract_jurisdiction = form(
  v.object({
    contract_id: RemoteNumberSchema,
    property_id: RemoteNumberSchema,
    court_id: RemoteNumberSchema,
  }),
  async (input) => {
    ...
    try {
      await query_builder
        ...
        .set({ court_id: input.court_id, updated_at: now })
      ...
      error(500, "Error al actualizar la jurisdicción")
    }
  },
)
