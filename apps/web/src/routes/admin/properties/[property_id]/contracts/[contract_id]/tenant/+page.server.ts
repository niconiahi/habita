...
import { RECEIPT_TYPE, get_receipt_types } from "$lib/receipt_type";
...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  const contract_id = v.parse(ForceNumberSchema, params.contract_id, {
  ...
  if (!contract) {
    error(404, `contract does not exist for id ${contract_id}`);
  }
  if (!property) {
    error(400, "not found");
  }
  const latest_period = contract.periods.sort((a, b) => {
    ...
    return new Date(b_start_date).getTime() - new Date(a_start_date).getTime();
  })[0];
  const two_months_ago = startOfMonth(subMonths(new Date(), 2));
  ...
  const receipt_types = get_receipt_types().filter((type) => {
...
export const actions: Actions = {
  [ACTION.UPLOAD_RECEIPT]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    const property_id = v.parse(ForceNumberSchema, params.property_id, {
