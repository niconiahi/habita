...
export async function invite_owner(form_data: FormData) {
  const email = v.parse(v.pipe(v.string(), v.email()), form_data.get("email"));
  ...
  const subject = "Estas siendo invitado como dueño de una propiedad";
  const url = new URL(
...
async function send_owner_invite({
  ...
  try {
    ...
    throw new Error("there was an error while sending the owner invite", {
      cause: error
    });
  }
}
