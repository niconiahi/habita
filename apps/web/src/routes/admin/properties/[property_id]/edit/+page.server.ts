...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  if (!property) {
    error(404, `property does not exist for id ${property_id}`);
  }
  return { property };
};
...
export const actions: Actions = {
  ...
  [ACTION.UPDATE_ROOM]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    if (!has_edit_access(locals.user.accesses)) {
  ...
  [ACTION.UPDATE_ROOM_POSITIONS]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    if (!has_edit_access(locals.user.accesses)) {
  ...
  [ACTION.DESTROY_ROOM]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    if (!has_edit_access(locals.user.accesses)) {
  ...
  [ACTION.UPDATE_SERVICE]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    if (!has_edit_access(locals.user.accesses)) {
    ...
    try {
      ...
      return { error: "Sólo puede haber un servicio de cada tipo" };
    }
  },
  [ACTION.DESTROY_SERVICE]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    if (!has_edit_access(locals.user.accesses)) {
  ...
  [ACTION.CREATE_PROPERTY_FILE]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    if (!has_edit_access(locals.user.accesses)) {
  ...
  [ACTION.INVITE_OWNER]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    if (!has_edit_access(locals.user.accesses)) {
  ...
  [ACTION.UPDATE_DESTINIES]: async ({ request, locals, params }) => {
    if (!locals.user) {
      redirect(302, "/auth/google");
    }
    if (!has_edit_access(locals.user.accesses)) {
