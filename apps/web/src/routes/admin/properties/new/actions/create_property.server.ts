...
import { PROPERTY_TYPE, PropertyTypeSchema } from "$lib/property_type";
...
export async function create_property(form_data: FormData) {
  ...
  const type = v.parse(PropertyTypeSchema, Number(form_data.get("type")));
  const destinies = v.parse(
    v.array(PropertyDestinySchema),
    form_data.getAll("destiny").map(Number)
  );
  const now = new Date().toISOString();
  const property = await query_builder.transaction().execute(async (tx) => {
    ...
    if (type === PROPERTY_TYPE.DEPARTMENT) {
      const unit_ = v.parse(v.string(), form_data.get("unit"));
      unit = unit_;
    }
    const property = await tx
      ...
      .executeTakeFirstOrThrow();
    const user_id = v.parse(ForceNumberSchema, form_data.get("user_id"));
    await tx
      .insertInto("access")
      .values({
