<script lang="ts">
  ...
  const has_editable = $derived(
    data.contracts.some((contract) => contract.state === CONTRACT_STATE.EDITING)
  );
</script>
...
<Content.Root>
  ...
  <Content.Section>
    ...
    >
      <Content.Section>
        <Formulary.Field>
          <Formulary.Label for="state">Estado</Formulary.Label>
          <Formulary.Select id="state" name="state" value={data.state}>
    ...
    <Table.Root>
      ...
      <Table.Body>
        {#each data.contracts as contract (contract.id)}
          {@const is_editable = contract.state === CONTRACT_STATE.EDITING}
          <Table.Row>
            <Table.Cell>{display_location(contract.location)}</Table.Cell>
            <Table.Cell>{format_end_date(contract.end_date)}</Table.Cell>
            {#if has_editable}
              <Table.Cell>
                {#if is_editable}
                  <a
