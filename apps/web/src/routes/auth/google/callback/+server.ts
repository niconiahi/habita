...
export const GET: RequestHandler = async ({ url, cookies }) => {
  ...
  const stored_code_verifier = cookies.get(GOOGLE_CODE_VERIFIER_COOKIE_NAME);
  ...
  try {
    ...
    const google_user = v.parse(GoogleUserSchema, await response.json());
    ...
    if (existing_user) {
      await create_session_and_set_cookies(existing_user.id, cookies);
...
async function create_session_and_set_cookies(
  ...
  cookies.set(SESSION_COOKIE_NAME, session_token, SESSION_COOKIE_OPTIONS);
