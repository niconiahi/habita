  ...
  () => {
    ...
    test("2. Generates PDF and sees all validation errors", async ({
      ...
      await Promise.all([
        ...
        page.click(
          'button:has-text("Generar contrato")',
        ),
      ])
      ...
      await expect(
        page.locator(
          'text=Falta asignar un propietario',
        ),
      ).toBeVisible()
      await expect(
        page.locator(
          'text=Falta asignar un inquilino',
        ),
      ).toBeVisible()
      await expect(
        page.locator(
          'text=Falta la fecha de inicio',
        ),
      ).toBeVisible()
      await expect(
        page.locator(
          'text=Falta la fecha de finalización',
        ),
      ).toBeVisible()
      await expect(
        page.locator(
          'text=Falta el tipo de escalación',
        ),
      ).toBeVisible()
      await expect(
        page.locator(
          'text=Falta la duración de escalación',
        ),
      ).toBeVisible()
      await expect(
        page.locator(
          'text=Falta el porcentaje de mora',
        ),
      ).toBeVisible()
    })
  },
