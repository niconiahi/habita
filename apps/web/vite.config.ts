...
function font_hasher(): Plugin {
  return {
    name: "font-hasher",
    writeBundle() {
      const source_path = path.join(process.cwd(), FONTS_DIR)
      const build_assets_path = path.join(process.cwd(), BUILD_DIR, "assets")
      if (!fs.existsSync(source_path)) {
        console.warn("Fonts directory not found:", source_path)
        return
      }
      const font_files = fs.readdirSync(source_path).filter(file =>
      ...
      for (const font_file of font_files) {
        ...
        const target_path = path.join(build_assets_path, hashed_file_name)
      ...
      const css_files = fs.readdirSync(build_assets_path).filter(file => file.endsWith(".css"))
      for (const css_file of css_files) {
        const css_path = path.join(build_assets_path, css_file)
        let css_content = fs.readFileSync(css_path, "utf-8")
        let modified = false
        for (const [original_file, hashed_file] of Object.entries(font_mapping)) {
          const original_url = `/fonts/${original_file}`
          const hashed_url = `/assets/${hashed_file}`
          if (css_content.includes(original_url)) {
            css_content = css_content.replace(new RegExp(original_url, "g"), hashed_url)
            modified = true
          }
        }
        if (modified) {
