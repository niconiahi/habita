...
The orchestration script `bin/map/argentina/fetch_zones` runs the SQL query against Nominatim, then pipes the raw TSV through both awk functions to produce the final `data.tsv`.
...
- `bin/map/argentina/fetch_zones` — orchestrates SQL extraction + awk transformation
- `bin/map/argentina/fetch_tiles` — generates Argentina mbtiles from Geofabrik OSM extract
