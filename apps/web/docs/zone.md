...
The orchestration script `bin/zone/extract.sh` runs the SQL query against Nominatim, then pipes the raw TSV through both awk functions to produce the final `data.tsv`.
...
- `bin/zone/extract.sh` — orchestrates SQL extraction + awk transformation
