...
import { session_cookie } from "./session.server"
...
export async function validate_session_token(
  ...
  const session_expired =
    Date.now() >= session.expires_at.getTime()
  if (session_expired) {
    await query_builder
      .deleteFrom("session")
      .where("id", "=", session.id)
  ...
  const renew_session =
    Date.now() >=
    session.expires_at.getTime() - DAY_IN_MS * 15
  if (renew_session) {
    session.expires_at = new Date(
      Date.now() + DAY_IN_MS * 30,
    )
    await query_builder
      .updateTable("session")
...
export type session_validation_result = Awaited<
  ReturnType<typeof validate_session_token>
>

export async function invalidate_session(
  session_id: string,
) {
  await query_builder
    .deleteFrom("session")
...
export async function get_auth(request: Request) {
  const session_token = await session_cookie.parse(
    request.headers.get("Cookie"),
  )
