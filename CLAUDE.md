...
- **Error handling** — Two layers: (1) utility functions throw custom `Error` subclasses with a `.type` discriminant — callers catch and match with `instanceof` + `.type`; (2) actions use SvelteKit-native `fail()` and `redirect()` directly. Two fail shapes: `fail(400, { errors: v.flatten(issues) })` for validation, `fail(400, { message: "..." })` for execution errors. One action per form. Callers just `return action(form_data)`. Templates use `form?.errors?.nested?.field?.[0]` and `form?.message`. Multiple DB operations = transaction. Read [`.claude/skills/error_handling/SKILL.md`](.claude/skills/error_handling/SKILL.md) for the complete reference.

- **Data validation** — Always validate non-type-safe sources (`response.json()`, `formData.get()`, `searchParams.get()`, `cookies.get()`, `localStorage.getItem()`, `JSON.parse()`) with Valibot. Inside actions: use `normalize_input` + `v.safeParse`. Outside: use `v.parse`. `JSON.parse` always inside `v.transform` or try/catch. Locally constructed data uses `satisfies`, not `v.parse`. Read [`.claude/skills/data_validation/SKILL.md`](.claude/skills/data_validation/SKILL.md).
